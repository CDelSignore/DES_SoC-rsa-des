...
class des():
    ...
    def substitute(self, d_e):#Substitute bytes using SBOX
        ...
        # Pass each subblock and the corresponding round into the compute function to get a long string
        result_string = ""
        for i in range(len(subblocks)):
            result_string += self.compute_s_box(subblocks[i], i)

        # Convert our string into individual digits for result block
        result = [int(b) for b in result_string]
    ...
    def compute_s_box(self,block,round):
        # Let i equal the decimal representation of the binary number made from the first and last bits of block
        i = 2*block[0] + block[5]

        # Let j equal the decimal representation of the binary number made from the middle four bits of block
        j = 8*block[1] + 4*block[2] + 2*block[3] + block[4]

        # Get the decimal value in the corresponding row and col in this round's s-box
        dec = S_BOX[round][i][j]
        
        # Return the value as a 4-digit binary string
        return binvalue(dec, 4)
